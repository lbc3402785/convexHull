...
bool DlibFace(Mat img, vector<Rect> &rectangles, vector<vector<Point2f>> &keypoints)
{
    ...
    keypoints = vector<vector<Point2f>>(faces.size());

	for (size_t i = 0; i < shapes.size(); i++)
	{
		auto shape = shapes[i];
		int N = shape.num_parts();
        keypoints[i] = vector<Point2f>(N);
		for (size_t j = 0; j < N; j++)
		{
			dlib::point p = shape.part(j);

            keypoints[i][j] = Point2f(p.x(), p.y());
		}
